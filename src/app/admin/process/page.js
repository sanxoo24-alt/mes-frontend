...
export default function ProcessPage() {
  ...
  const [form, setForm] = useState({
    process_code: '', process_name: '', department_id: '', tat: '', note: ''
  });
  ...
  const filtered = processes.filter(p =>
    p.process_code.toLowerCase().includes(keyword.toLowerCase()) ||
    p.process_name.toLowerCase().includes(keyword.toLowerCase())
  );

  const openNew = () => {
    setEditTarget(null);
    setForm({ process_code: '', process_name: '', department_id: '', tat: '', note: '' });
    setError('');
    setModalOpen(true);
  };
  ...
  return (
    <div className="relative min-h-full">
      <div className="flex items-center gap-2 text-sm text-slate-400 mb-6">
        <span>등록/관리</span>
        <span>&gt;</span>
        <span className="text-slate-700 font-medium">공정 등록</span>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              <th className="px-5 py-3 text-left font-semibold text-slate-600">공정 코드</th>
              <th className="px-5 py-3 text-left font-semibold text-slate-600">공정명</th>
              <th className="px-5 py-3 text-left font-semibold text-slate-600">부서</th>
              <th className="px-5 py-3 text-center font-semibold text-slate-600">TAT(시간)</th>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={7} className="text-center py-16 text-slate-400">불러오는 중...</td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={7} className="text-center py-16 text-slate-400">표시할 데이터가 없습니다.</td></tr>
            ) : (
              filtered.map((p) => (
                <tr key={p.id} className={`border-b border-slate-100 hover:bg-slate-50 transition-colors ${!p.is_active ? 'opacity-40' : ''}`}>
                  <td className="px-5 py-3 font-mono font-semibold text-slate-700">{p.process_code}</td>
                  <td className="px-5 py-3 font-medium text-slate-800">{p.process_name}</td>
                  <td className="px-5 py-3 text-slate-500">{p.department_name || p.department || '-'}</td>
                  <td className="px-5 py-3 text-center text-slate-600">{p.tat ?? '-'}</td>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">
                  공정코드 <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">TAT (시간)</label>
                <input
                  type="number"
                  min="0"
                  value={form.tat}
